...
function checkMcpServers() {
  ...
  // Also check enabledPlugins in settings.json (plugin-based MCP servers)
  let enabledPlugins = {};
  ...
  for (const server of REQUIRED_MCP_SERVERS) {
    const inPlugins = Object.entries(enabledPlugins)
      .some(([key, val]) => key.startsWith(server.config_key + '@') && val === true);
    if (!mcpServers[server.config_key] && !inPlugins) {
      missing.push({
        type: 'mcp_server',
        name: server.name,
